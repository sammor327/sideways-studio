...
- The champion and the runner up are the final, which is to say the match already loaded: set its winner under Match data › Match card and result and both graphics name the right player. Each has a **Winner** switch for pinning a side by hand.
- **The between-games portion runs itself.** Under the TAKE buttons there is now a **Between games** block: move the game wins as a game ends, press **Run**, and the game victory goes up naming whoever went ahead, the sideboards come in after it, and the graphic holds there for as long as the players take. Type both battlefields for the next game under Match data and the game intro follows on its own, runs for its five seconds and goes out. Nothing in the sequence waits for a TAKE, and nothing else on air is touched: the in-game overlay, the score bug and the ticker stay exactly as they were.
- The two holds (**Victory** and **Intro**, in seconds) are set in the same block. **Next** skips a step, which is what to press when a player keeps the same battlefield and there is nothing new for the run to see. **Stop** ends it and takes its graphics down. The three steps light up as the run reaches them.
- Only the two battlefields ride on air with the run: a name half typed in preview while a game is being played cannot reach the screen on the back of it.
- **The game intro now takes the whole game window.** On the rows overlay it fills the game area edge to edge, with no strip of the feed showing above, below or beside it (Sam). The two pages divide the window's width between them and everything on a page is scaled to its height, so the intro is bigger on every overlay: a third bigger in the dual columns, a quarter bigger in the portrait pillars.
- **A new slate screen: Round board.** Pick it under Graphic features › Slate and leave the slate up: it follows a round through on its own. While tables are out it shows what is still being played; as results land it turns between those and the results so far; once the round is in it holds on the results; and the moment the next round's pairings are loaded it becomes the pairings. All of it off the round already loaded from the Tournament platform tab, with nothing more to type.
- **The Graphics list is in eight sections instead of five.** New: **Pre-game** (Standings, Head to head VS, Decklists side by side, Match card, Player profile), **Stats** (Matchup matrix, Legend distribution), **Victory** (the three above plus the Result strip) and **Archive** (Score bug, Arena score bug, In-game overlay 1v1), with 1v1, 2v2, Between games and Other keeping the rest. Favorites is still at the top and a starred graphic still moves there from wherever it is listed.
- Every graphic's description now sits directly under its name. It used to drift down the row when the line was short and the picture beside it was taller than both.
- **A graphic's link works without the slash on the end.** A browser source pasted as `/scenes/igorows` (rather than `/scenes/igorows/`) used to load nothing at all, with no way to tell a missing slash from a broken graphic (Sam). It now goes to the right page. Every link the app hands out, and every file each of those pages loads, is checked by the tests from now on.
- Pulling the matchup matrix from Rift Registry was checked end to end from a plain machine: the list of events and an event's own file are public, asked for the same way the card download asks, and nothing of the machine doing the asking goes with the request. A venue that cannot reach the site at all can still paste the numbers in.
